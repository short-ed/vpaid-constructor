...
function uploadSlideImage(e: any, slideId: number) {
...
function copySlide(slideId: number) {
  const index = localSlides.value.findIndex(slide => slide.id === slideId)
...
<template>
  <div>
    <button type="button" class="mb-4 w-full h-9 bg-primary text-xs  text-primary-foreground rounded-2 flex items-center justify-center" @click="createSlide">
      Добавить слайд
    </button>
        ...
        <div class="p-2 bg-[#f1f1f1] rounded-2 grid gap-1">
          <div v-for="field in visibleFields">
            <div v-if="field === SlideField.Title">
              <input v-model="element[field]" class="w-full text-sm bg-background px-2 rounded-2 text-foreground h-8">
            </div>
            ...
            <div v-else-if="field === SlideField.Description">
              <input v-model="element[field]" class="w-full bg-background text-xs px-2 rounded-2 text-center h-8 flex items-center justify-center">
            </div>
            <div v-else-if="field === SlideField.Button">
              <div class="w-full h-9 bg-primary text-xs  text-primary-foreground rounded-2 flex items-center justify-center  cursor-default">
                {{ textOnButton }}
              </div>
            </div>
            <div v-else-if="field === SlideField.Link">
              <div class="text-center text-primary flex items-center justify-center text-xs underline hover:decoration-none  cursor-default">
                {{ textOnLink }}
              </div>
            </div>
            <div v-else>
          ...
          <input v-model="element.url" class="w-full text-sm bg-background px-2 rounded-2 text-foreground h-8 mt-2" type="text" placeholder="URL">
